...
public class HealthController {
    ...
    @SwaggerApiSpec(
            summary = "건강 데이터 업로드",
            description = "모바일에서 수집된 걸음 수, 심박수 등의 건강 데이터를 서버에 저장합니다.",
            successMessage = "건강 데이터 저장 및 기기 연동 성공",
            errors = {ErrorCode.ENTITY_NOT_FOUND, ErrorCode.INVALID_INPUT_VALUE, ErrorCode.INTERNAL_SERVER_ERROR}
    )
    @PostMapping("/data")
    ...
    @SwaggerApiSpec(
            summary = "삼성 헬스 연동 상태 등록",
            description = "앱에서 확인된 삼성 헬스 연동 권한 상태를 서버에 등록합니다.",
            successMessage = "연동 상태 등록 성공",
    ...
    public RestApiResponse<HealthConnectionResponseDTO> registerConnection(
        ...
        HealthConnectionResponseDTO response = healthConnectionService.registerConnection(userDetails.getId().intValue(), request);
        return RestApiResponse.success(response);
    }

    @SwaggerApiSpec(
            summary = "삼성 헬스 연동 상태 조회",
            description = "현재 사용자의 삼성 헬스 연동 여부 및 마지막 동기화 시각을 조회합니다.",
            successMessage = "연동 상태 조회 성공",
            errors = {ErrorCode.ENTITY_NOT_FOUND}
    )
    ...
    public RestApiResponse<HealthConnectionResponseDTO> getConnectionStatus(
        ...
        HealthConnectionResponseDTO response = healthConnectionService.getConnectionStatus(userDetails.getId().intValue(), provider);
        return RestApiResponse.success(response);
    }
}
