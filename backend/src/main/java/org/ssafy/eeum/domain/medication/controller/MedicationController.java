...
public class MedicationController {
    ...
    public ResponseEntity<List<Long>> createMedicationPlans(
        ...
        List<Long> ids = medicationService.createMedicationPlans(familyId, requests);
    ...
    public ResponseEntity<MedicationResponse> getMedicationPlan(
        ...
        MedicationResponse response = medicationService.getMedicationPlan(medicationId);
        return ResponseEntity.ok(response);
    }
    
    @Operation(summary = "가족별 복약 정보 조회", description = "가족 ID로 해당 가족의 모든 복약 정보를 조회합니다.")
    @GetMapping
    public ResponseEntity<List<MedicationResponse>> getMedicationPlans(@PathVariable Long familyId) {
